...
function statusKeyOf(order) {
  return (
    ORDER_STATUS_DISPLAY_MAP[order.order_status]?.key ?? order.order_status
  );
}
...
function NewOrderModal({ isOpen, onClose }) {
  ...
  async function handleSubmit() {
    setFormError("");
    try {
      orderSchema.validateSync(
        { customerId, storeId, lines },
        { abortEarly: true },
      );
      ...
      <div className="row g-3 mb-3">
        ...
        <div className="col-md-6">
          ...
          <select
            ...
            value={storeId ?? ""}
            onChange={(event) =>
              setStore(event.target.value ? Number(event.target.value) : null)
            }
          >
...
function AdminOrders() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (filters.store) {
      list = list.filter(
        (order) => String(order.store_id) === String(filters.store),
      );
    }
    if (filters.dateFrom) {
      list = list.filter(
        (order) => (order.order_tms ?? "").slice(0, 10) >= filters.dateFrom,
      );
    }
    if (filters.dateTo) {
      list = list.filter(
        (order) => (order.order_tms ?? "").slice(0, 10) <= filters.dateTo,
      );
    }
    return list;
  }, [orders, filters]);
  ...
  const pagedOrders = pageItems(filteredOrders);
    ...
    <div>
      ...
      <div className="d-flex flex-wrap align-items-center gap-2 mb-3">
        <select
          ...
          value={filters.store}
          onChange={(event) =>
            setFilters("orders", { store: event.target.value })
          }
        >
        ...
        <input
          ...
          onChange={(event) =>
            setFilters("orders", { dateFrom: event.target.value })
          }
        />
        ...
        <input
          ...
          onChange={(event) =>
            setFilters("orders", { dateTo: event.target.value })
          }
        />
...
function CustomerOrders() {
  ...
  const tabFiltered =
    activeTab === "ALL"
      ? myOrders
      : myOrders.filter((order) => statusKeyOf(order) === activeTab);
  ...
  const pagedOrders = pageItems(tabFiltered);
        ...
        {CUSTOMER_TABS.map((tab) => (
          <li className="nav-item" key={tab}>
            ...
            >
              {tab === "ALL"
                ? "All"
                : (ORDER_STATUS_DISPLAY_MAP[tab]?.label ?? tab)}
            </button>
          </li>
        ))}
      </ul>
