  ...
  return (
    <div className="low-stock-card">
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={PackageCheck}
          heading="All stock levels are healthy"
        />
      ) : (
        <ul className="low-stock-card__list">
          {lowStockRows.map((row) => (
            <li
              key={row.id ?? row.inventory_id}
              className="low-stock-card__row"
            >
              <TriangleAlert
                className="low-stock-card__icon"
                size={16}
                strokeWidth={1.75}
                aria-hidden="true"
              />
              ...
              <div className="low-stock-card__info">
                <span className="low-stock-card__name">
                  {row.product?.product_name ?? 'Unknown product'}
                </span>

                <span className="low-stock-card__store">
                  {row.store?.store_name ?? 'Unknown store'}
                </span>
              </div>

              <span className="low-stock-card__qty tabular-nums">
                {row.product_inventory} left
              </span>
            </li>
          ))}
        </ul>
